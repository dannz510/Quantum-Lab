...
import { Play, Pause, RotateCcw, Save, Settings, Info, Loader2, Sparkles, X, Activity } from 'lucide-react';
...
export const PendulumLab: React.FC<PendulumLabProps> = ({ lang }) => {
  ...
  const animateRef = (timestamp: number) => {
    ...
    if (Math.floor(timestamp) % 5 === 0) { 
       setDataPoints(prev => {
          const newPts = [...prev, { time: performance.now(), value: next.theta, secondaryValue: next.omega }];
          return newPts.slice(-200);
       });
    }
  ...
  // Modern Phase Space Visualization (Phase Portrait)
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center opacity-30 pointer-events-none">
         <svg width="100%" height="100%" viewBox="-150 -100 300 200">
            <path 
              d={`M ${dataPoints.map(p => `${p.value * scale} ${-(p.secondaryValue || 0) * scale * 0.5}`).join(' L ')}`}
              fill="none"
              stroke="#60a5fa"
              strokeWidth="2"
            />
  ...
  return (
    <div className="h-full grid grid-cols-1 lg:grid-cols-12 gap-4 p-4 overflow-hidden">
      ...
      <div className="lg:col-span-3 bg-lab-card border border-slate-700 rounded-2xl p-4 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="flex justify-between text-sm text-slate-400 mb-1">
              {t('Length (L)', 'Chiều Dài (L)')} <span>{length.toFixed(2)} m</span>
            </label>
            <input 
          ...
          <div>
            <label className="flex justify-between text-sm text-slate-400 mb-1">
              {t('Mass (m)', 'Khối Lượng (m)')} <span>{mass.toFixed(1)} kg</span>
            </label>
            <input 
          ...
          <div>
            <label className="flex justify-between text-sm text-slate-400 mb-1">
              {t('Initial Angle', 'Góc Ban Đầu')} <span>{initialAngle}&deg;</span>
            </label>
            <input 
            ...
            {initialAngle > 15 && (
               <p className="text-[10px] text-amber-500 mt-1 flex items-center gap-1">
                 <Info size={10} /> {t('Large angle approx.', 'Góc lớn (Phi tuyến)')}
               </p>
            )}
          </div>

          <div>
             <label className="block text-sm text-slate-400 mb-2">{t('Bob Material', 'Vật Liệu Quả Nặng')}</label>
          ...
          <div>
            <label className="flex justify-between text-sm text-slate-400 mb-1">
              {t('Air Resistance', 'Sức Cản Không Khí')} <span>{(damping * 100).toFixed(0)}%</span>
            </label>
            <input 
        ...
        <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50 space-y-2">
           <div className="flex justify-between text-xs">
             <span className="text-slate-500">{t('Ideal Period (T₀)', 'Chu Kỳ Lý Tưởng (T₀)')}</span>
             <span className="text-slate-300 font-mono">{(2 * Math.PI * Math.sqrt(length/9.81)).toFixed(3)} s</span>
           </div>
           <div className="flex justify-between text-xs">
             <span className="text-slate-500">{t('Actual Period (T)', 'Chu Kỳ Thực Tế (T)')}</span>
      ...
      <div className="lg:col-span-6 flex flex-col gap-4">
        <div className="flex-1 bg-slate-900 rounded-2xl border border-slate-700 relative overflow-hidden flex items-center justify-center shadow-inner shadow-black/40">
            ...
            <div className="relative w-full h-full flex justify-center pt-10 origin-top">
                <svg width="100%" height="100%" viewBox="-150 -50 300 400" className="overflow-visible z-10">
            ...
            <div className="absolute bottom-6 flex items-center gap-4 bg-slate-800/80 backdrop-blur-lg p-2 rounded-full border border-slate-600 shadow-xl z-20">
      ...
      <div className="lg:col-span-3 flex flex-col gap-4">
        <div className="bg-lab-card border border-slate-700 rounded-2xl p-4 flex-1 overflow-hidden flex flex-col">
           <h3 className="font-bold text-slate-300 mb-4 flex items-center gap-2">
             <Activity size={16} /> {t('Analysis', 'Phân Tích')}
           </h3>
           
           {/* Modern Phase Space */}
           <div className="h-32 bg-slate-900 rounded-lg border border-slate-800 mb-4 relative overflow-hidden">
              {renderPhaseSpace()}
              <div className="absolute bottom-1 right-2 text-[10px] text-slate-500 font-mono">Phase Space (θ vs ω)</div>
           </div>
           ...
           <div className="space-y-2 mb-4">
             <div>
               <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                 <span>{t('Kinetic', 'Động Năng')}</span> <span>{energy.ke.toFixed(2)} J</span>
               </div>
               <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                 <div className="h-full bg-green-500 transition-all duration-75 shadow-[0_0_10px_rgba(34,197,94,0.5)]" style={{ width: `${Math.min((energy.ke / 5) * 100, 100)}%` }}></div>
               </div>
             </div>
             <div>
               <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                 <span>{t('Potential', 'Thế Năng')}</span> <span>{energy.pe.toFixed(2)} J</span>
               </div>
               <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                 <div className="h-full bg-blue-500 transition-all duration-75 shadow-[0_0_10px_rgba(59,130,246,0.5)]" style={{ width: `${Math.min((energy.pe / 5) * 100, 100)}%` }}></div>
               </div>
             </div>
           </div>
           ...
           <div className="mt-auto border-t border-slate-700 pt-4">
              {aiAnalysis ? (
                ...
                <button 
                  ...
                  className="w-full py-3 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl text-white font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-all"
                >
                  {isAnalyzing ? <Loader2 className="animate-spin" size={16} /> : <Sparkles size={16} />}
                  {t('Generate AI Report', 'Tạo Báo Cáo AI')}
                </button>
              )}
           </div>
        </div>
